...
in_dir   <- file.path(base_dir, "2_Exp_data")
out_dir  <- file.path(base_dir, "2_1_DGE_limma")
...
# Minimal QC print for any matrix
...
# DEG summary for a limma topTable output
...
run_limma_pair <- function(file_ad, file_ctl, prefix) {
  ...
  # 2) Print input QC (sanity checks)
  print_qc(mat_ad,  "QC AD")
  print_qc(mat_ctl, "QC Control")
  
  # 3) Harmonize genes (should be identical across paired files)
  ...
  # 4) Combine expression and build design (two-group comparison)
  ...
  # 8) Print a concise DEG count summary (based on PADJ_CUT and LFC_CUT)
...
print(summary_df)
# A tibble: 5 × 4
#Contrast          Significant Upregulated Downregulated
#   <chr>              <int>       <int>         <int>
#1 Mayo_CRB            1838         486          1352
#2 Mayo_TC              932         783           149
#3 ROSMAP_HCN           162          86            76
#4 ROSMAP_DLPFC         143          77            66
#5 ROSMAP_PCC           102          27            75
